...
use hartree::cc::{
    CcsdOptions, frozen_core_orbitals, rccsd_spin_adapted, rccsd_spin_orbital, rhf_mp2,
};
...
fn ccsd_iteration0_equals_mp2() {
    ...
    for (name, basis) in [("h2", "6-31g"), ("water", "sto-3g"), ("water", "6-31g")] {
        ...
        for n_frozen in [0, frozen_core_orbitals(&mol)] {
            ...
            let cc = rccsd_spin_adapted(&provider, &scf, n_frozen, &opts);
...
#[test]
fn spin_adapted_matches_spin_orbital() {
    let geoms: Geometries = serde_json::from_str(GEOMETRIES_JSON).unwrap();
    let opts = CcsdOptions::default();
    ...
    for (name, basis) in [
        ...
        let scf = converged_rhf(&provider, &mol);
        for n_frozen in [0, frozen_core_orbitals(&mol)] {
            let so = rccsd_spin_orbital(&provider, &scf, n_frozen, &opts);
            let sa = rccsd_spin_adapted(&provider, &scf, n_frozen, &opts);
            assert!(
                so.converged && sa.converged,
                "{name}/{basis}: CCSD not converged"
            );
            let d = sa.correlation_energy - so.correlation_energy;
            worst = worst.max(d.abs());
            eprintln!(
                "5b vs 5a {name}/{basis} (frozen {n_frozen}): {:.12} vs {:.12} (Δ {:.2e})",
                sa.correlation_energy, so.correlation_energy, d
            );
            assert!(
                d.abs() < 1e-10,
                "{name}/{basis} (frozen {n_frozen}): 5b {:.12} vs 5a {:.12} (Δ {:.2e})",
                sa.correlation_energy,
                so.correlation_energy,
                d
            );
        }
    }
    eprintln!("5b vs 5a: worst ΔEcorr = {worst:.2e}");
}
...
fn check_oracle(bases: &[&str]) -> usize {
    ...
    eprintln!("CCSD(5b): {checked} oracle entries matched; worst ΔEcorr = {worst:.2e}");
    checked
}

#[test]
fn rccsd_spin_adapted_matches_oracle_fast() {
...
#[test]
#[ignore = "cc-pVDZ in debug is slow; run with --release"]
fn rccsd_spin_adapted_matches_oracle_ccpvdz() {
